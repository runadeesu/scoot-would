...
# (google/fonts folder, file name, local file name)
FONTS = [
    ("barlow", "Barlow-Regular.ttf", "Barlow-Regular.ttf"),
    ("barlow", "Barlow-SemiBold.ttf", "Barlow-SemiBold.ttf"),
    ("barlow", "Barlow-Bold.ttf", "Barlow-Bold.ttf"),
    ("barlowcondensed", "BarlowCondensed-ExtraBoldItalic.ttf", "BarlowCondensed-ExtraBoldItalic.ttf"),
    ("barlow", "OFL.txt", "OFL.txt"),
    # Japanese UI text (fallback for every glyph Barlow does not have)
    ("mplus1p", "MPLUS1p-Regular.ttf", "MPLUS1p-Regular.ttf"),
    ("mplus1p", "MPLUS1p-Bold.ttf", "MPLUS1p-Bold.ttf"),
    ("mplus1p", "MPLUS1p-Black.ttf", "MPLUS1p-Black.ttf"),
    ("mplus1p", "OFL.txt", "OFL-MPLUS1p.txt"),
]
...
def main():
    ...
    for family, fname, local in FONTS:
        out = os.path.join(FONT_DIR, local)
    ...
    manifest.append("| fonts/MPLUS1p-*.ttf | https://github.com/google/fonts/tree/main/ofl/mplus1p (M+ Fonts Project) | SIL OFL 1.1 (fonts/OFL-MPLUS1p.txt) |")
    manifest += [
        "",
        "## Rider (human)",
        "",
    ...
    with open(os.path.join(ROOT, "assets", "SOURCES.md"), "w") as f:
        ...
        f.write("The UI typefaces are Barlow by Jeremy Tribby and, for Japanese, M PLUS 1p by the M+ Fonts Project, both under the SIL Open Font License 1.1.\n")
